...
class UMIModel(nn.Module):
    ...
    def __init__(
        ...
        model_dir: Path = Path("models/UMI"),
        data_dir: Path = Path("data/UMI"),
        storage_url: str = "sqlite:///models/UMI/umi_hp_optimization.db",
        warm_start : bool= False,
        warm_training_epochs : int = self.epochs,
        **hparams,
    ):
        ...
        self.model_dir = model_dir / freq / self._clock().strftime("%Y-%m-%dT%H-%M-%S")  # model directory with timestamp
        ...
        self.warm_training_epochs = warm_training_epochs
    ...
    # ---------------- fit -------------------------------------------- #
    def fit(self, data_dict: Dict[str, "pd.DataFrame"], warm_start: bool, n_epochs: int):
        """
        ...
        def objective(trial):
            ...
            val = self._train(loader_train, loader_valid, n_epochs=15)  # 15-epoch eval
        ...
        for k in sorted(new_data_dict):
            if k not in self._universe:
                if len(self._universe) < self.I:        # capacity check
                    ...
                    self.fit(new_data_dict, warm_start=self.warm_start, n_epochs=self.warm_training_epochs)               # rebuild with larger I. fit will expand the universe
                    return
                else:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--model-dir", default=DEFAULT_MODELDR, help="directory where the model is saved")
